...
def boxes3d_kitti_camera_to_imageboxes(boxes3d, calib, image_shape=None):
    ...
    """
    # 1) 角点：跟随输入类型（torch -> torch, numpy -> numpy）
    corners3d = boxes3d_to_corners3d_kitti_camera(boxes3d)

    # 2) 投影到像素坐标
    pts_img, _ = calib.rect_to_img(corners3d.reshape(-1, 3))

    # 3) 统一成 torch.Tensor，在 GPU/CPU 上计算，避免 numpy 参与
    if isinstance(pts_img, np.ndarray):
        pts_img_t = torch.from_numpy(pts_img)
    else:
        pts_img_t = pts_img  # 已是 torch.Tensor

    # 确保设备一致（若 boxes3d 是 torch.Tensor 则跟随其 device，否则用 CPU）
    device = boxes3d.device if isinstance(boxes3d, torch.Tensor) else torch.device('cpu')
    pts_img_t = pts_img_t.to(device=device, dtype=torch.float32)

    corners_in_image = pts_img_t.view(-1, 8, 2)

    # 4) 用 torch 计算 min/max
    min_uv = torch.min(corners_in_image, dim=1).values  # (N, 2)
    max_uv = torch.max(corners_in_image, dim=1).values  # (N, 2)
    boxes2d_image_t = torch.cat([min_uv, max_uv], dim=1)  # (N, 4)

    # 5) 用 torch.clamp 做边界裁剪
    if image_shape is not None:
        boxes2d_image_t[:, 0] = torch.clamp(boxes2d_image_t[:, 0], min=0, max=image_shape[1] - 1)
        boxes2d_image_t[:, 1] = torch.clamp(boxes2d_image_t[:, 1], min=0, max=image_shape[0] - 1)
        boxes2d_image_t[:, 2] = torch.clamp(boxes2d_image_t[:, 2], min=0, max=image_shape[1] - 1)
        boxes2d_image_t[:, 3] = torch.clamp(boxes2d_image_t[:, 3], min=0, max=image_shape[0] - 1)

    # 6) 根据输入类型决定返回 torch 还是 numpy
    if isinstance(boxes3d, torch.Tensor):
        return boxes2d_image_t  # 继续保持 torch（可在 GPU）
    else:
        return boxes2d_image_t.detach().cpu().numpy()
