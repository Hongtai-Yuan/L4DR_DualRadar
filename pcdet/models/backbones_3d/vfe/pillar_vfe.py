...
class PFNLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.use_norm:
            # x: (N, P, C)  ->  xt: (N, C, P) 供 BatchNorm1d 使用
            xt = x.permute(0, 2, 1)
            # BatchNorm1d(3D) 用 (N, P) 作为“批量”维，要求 N*P > 1
            if xt.shape[0] * xt.shape[2] > 1:
                xt = self.norm(xt)
            # 否则跳过 BN（避免 "Expected more than 1 value per channel" 报错）
            x = xt.permute(0, 2, 1)
...
class PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        if type(num_point_features) != int:
...
class MME_PillarVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        lidar_voxel_features, lidar_voxel_num_points, lidar_coords = (
            batch_dict['lidar_voxels'],
            batch_dict['lidar_voxel_num_points'],
            batch_dict['lidar_voxel_coords'],
        )
        radar_voxel_features, radar_voxel_num_points, radar_coords = (
            batch_dict['radar_voxels'],
            batch_dict['radar_voxel_num_points'],
            batch_dict['radar_voxel_coords'],
        )
        ...
        # --- 基础统计 ---
        lidar_points_mean = (
                lidar_voxel_features[:, :, :3].sum(dim=1, keepdim=True)
                / lidar_voxel_num_points.type_as(lidar_voxel_features).view(-1, 1, 1).clamp(min=1)
        )
        radar_points_mean = (
                radar_voxel_features[:, :, :3].sum(dim=1, keepdim=True)
                / radar_voxel_num_points.type_as(radar_voxel_features).view(-1, 1, 1).clamp(min=1)
        )
        ...
        # --- 组合 Radar 主导的共同特征 com_features ---
        len_radar = int(radar_voxel_num_points.max()) if radar_voxel_num_points.numel() > 0 else 1
        com_features = torch.zeros(
            (len(radar_voxel_num_points), len_radar, self.num_point_features_r), device=device
        )

        for i in range(len_radar):
            now_feature_idx = 0
            valid_mask = radar_voxel_num_points[common_R] >= (i + 1)
            valid_common_R = common_R[valid_mask]
            valid_common_L = common_L[valid_mask]

            # 1) radar 原始 xyz（宽度 3）
            w = 3
            com_features[:, i, now_feature_idx: now_feature_idx + w] = radar_voxel_features[:, i, :3]
            now_feature_idx += w

            # 2) LiDAR 侧 “强度/其他”体素内均值（动态宽度）
            if valid_common_L.numel() > 0:
                extraF_L = (
                        lidar_voxel_features[valid_common_L, :, 3:].sum(dim=1)
                        / lidar_voxel_num_points[valid_common_L].type_as(lidar_voxel_features).view(-1, 1).clamp(min=1)
                )
                extraF_L = torch.nan_to_num(extraF_L, nan=0.0, posinf=0.0, neginf=0.0)
                w = extraF_L.size(-1)
                if w > 0:
                    com_features[valid_common_R, i, now_feature_idx: now_feature_idx + w] = extraF_L
                    now_feature_idx += w

            # 3) radar -> lidar cluster（宽度 3）
            if valid_common_L.numel() > 0:
                common_lidar_points_mean = (
                        lidar_voxel_features[valid_common_L, :, :3].sum(dim=1)
                        / lidar_voxel_num_points[valid_common_L].type_as(lidar_voxel_features).view(-1, 1).clamp(min=1)
                )
                radartolidar_f_cluster = radar_voxel_features[valid_common_R, i, :3] - common_lidar_points_mean
                w = radartolidar_f_cluster.size(-1)
                com_features[valid_common_R, i, now_feature_idx: now_feature_idx + w] = radartolidar_f_cluster
                now_feature_idx += w

            # 4) radar f_center（宽度 3）
            w = radar_f_center.size(-1)
            com_features[:, i, now_feature_idx: now_feature_idx + w] = radar_f_center[:, i]
            now_feature_idx += w

            # 5) radar f_cluster（宽度 3）
            w = radar_f_cluster.size(-1)
            com_features[:, i, now_feature_idx: now_feature_idx + w] = radar_f_cluster[:, i]
            now_feature_idx += w

            # 6) radar 其他通道（去掉 xyz 后的剩余）
            w = radar_voxel_features.shape[-1] - 3
            if w > 0:
                com_features[:, i, now_feature_idx: now_feature_idx + w] = radar_voxel_features[:, i, 3:]
                now_feature_idx += w

        # 清理 NaN/Inf，避免后续 Linear/BN 触发钩子
        com_features = torch.nan_to_num(com_features, nan=0.0, posinf=0.0, neginf=0.0)

        # --- LiDAR 扩展特征 l_ex_features ---
        len_lidar = int(lidar_voxel_num_points.max())
        l_ex_features = torch.zeros(
            (len(lidar_voxel_num_points), 32, self.num_point_features_l), device=device
        )
        now_feature_idx = 0

        # LiDAR 原始
        w = lidar_voxel_features.shape[-1]
        l_ex_features[:, :, now_feature_idx: now_feature_idx + w] = lidar_voxel_features
        now_feature_idx += w

        # LiDAR f_cluster
        w = lidar_f_cluster.shape[-1]
        l_ex_features[:, :, now_feature_idx: now_feature_idx + w] = lidar_f_cluster
        now_feature_idx += w

        # LiDAR f_center
        w = lidar_f_center.shape[-1]
        l_ex_features[:, :, now_feature_idx: now_feature_idx + w] = lidar_f_center
        now_feature_idx += w

        # 计算 l2r：仅公共体素里、且 radar t=0 的样本
        if common_R.numel() > 0:
            ...
            if l2r_mask.any():
                ...
                common_radar_points_mean = (
                        (radar_voxel_features[l2r_com_R, :, :3] * mask).sum(dim=1, keepdim=True)
                        / num_valid.type_as(radar_voxel_features).view(-1, 1, 1).clamp(min=1)
                )
                lidartoradar_f_cluster = lidar_voxel_features[l2r_com_L, :, :3] - common_radar_points_mean

                w = lidartoradar_f_cluster.shape[-1]  # 3
                l_ex_features[l2r_com_L, :, now_feature_idx: now_feature_idx + w] = lidartoradar_f_cluster
                now_feature_idx += w

                # radar 其他通道的均值填充（后续如果有 radar 覆盖会再覆盖）
                extraFea_R = (
                        (radar_voxel_features[l2r_com_R, :, 3:] * mask).sum(dim=1, keepdim=True)
                        / num_valid.type_as(radar_voxel_features).view(-1, 1, 1).clamp(min=1)
                )
                extraFea_R = torch.nan_to_num(extraFea_R, nan=0.0, posinf=0.0, neginf=0.0)
                l_ex_features[l2r_com_L, :, now_feature_idx:] = extraFea_R
                # now_feature_idx 可不再累加，后续不再使用

        # 清理 NaN/Inf
        l_ex_features = torch.nan_to_num(l_ex_features, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        radar_features = radar_features * mask
