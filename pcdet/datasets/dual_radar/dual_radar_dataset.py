...
class DualradarDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.sim_info_path_list = [
            Path('/root/autodl-tmp/data/dualradar_0.01/'),
            Path('/root/autodl-tmp/data/dualradar_0.02/'),
            Path('/root/autodl-tmp/data/dualradar_0.03/'),
            Path('/root/autodl-tmp/data/dualradar_0.04/'),
        ]
        self.sim_info_path = Path('/root/autodl-tmp/data/dualradar_0.01/')
    ...
    def get_pc(self, idx):
        ARB2V = np.array([0.9981128011677526, 0.05916115557244023, 0.016455814060541557, 0.07800451346438697,
                          ...
                          0.015169806470300943, 0.022121191179620064, 0.9996402002082792,
                          -1.6030740415943632]).reshape([3, 4])
        if self.sensor == 'LiDAR':
            ...
            # 修改1  点数变为5
            points_arbe = np.fromfile(str(arbe_file), dtype=np.float32).reshape(-1, number_of_channels)
            ...
            if self.sim_info_path_list is not None and self.use_fog == 2 and self.train:
                ...
                if aug < len(self.sim_info_path_list):
                    ...
                    self.fog_I = 0
            l_points = np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 6)

            radar_file = self.root_split_path / 'arbe' / ('%s.bin' % idx)
            assert radar_file.exists()
            points_arbe = np.fromfile(str(radar_file), dtype=np.float32).reshape(-1, 5)
            num_point = points_arbe.shape[0]
            points_arbe_hom = np.hstack((points_arbe[:, 0:3], np.ones((num_point, 1))))
            point_lidar = np.dot(points_arbe_hom, np.transpose(ARB2V))
            points_arbe[:, 0:3] = point_lidar
            return l_points, points_arbe
        return points
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in range(len(infos)):
            ...
            if self.sensor == 'LiDAR' or self.sensor == 'Radar':
                ...
                for i in range(num_obj):
                    # filename = '%s_%s_%d.bin' % (sample_idx, names[i], i)
                    lidar_filename = '%s_%s_%s_%d.bin' % ('velodyne', sample_idx, names[i], i)
                    radar_filename = '%s_%s_%s_%d.bin' % ('arbe', sample_idx, names[i], i)
                    ...
                    if (used_classes is None) or names[i] in used_classes:
                        ...
                        db_info = {'name': names[i], 'lidar_path': lidar_db_path, 'radar_path': radar_db_path,
                                   'image_idx': sample_idx, 'gt_idx': i,
                                   'box3d_lidar': gt_boxes[i], 'lidar_num_points_in_gt': lidar_gt_points.shape[0],
                                   'radar_num_points_in_gt': radar_gt_points.shape[0],
                                   'difficulty': difficulty[i], 'bbox': bbox[i], 'score': annos['score'][i]}
                        if names[i] in all_db_infos:
                            all_db_infos[names[i]].append(db_info)
                        else:
    ...
    def __getitem__(self, index):
        ...
        if "points" in get_item_list:
            if self.sensor == 'LiDAR' or self.sensor == 'Radar':
                ...
                # radar_points[:,:3] = calib.rect_to_lidar(calib.lidar_to_rect(radar_points[:, 0:3]))
                if self.dataset_cfg.FOV_POINTS_ONLY:
...
def _safe_read_bin(self, path: Path, nchan: int):
    """稳健读取 .bin，保证 shape 正确、数值有限。返回 None 表示此帧作废。"""
    try:
        raw = np.fromfile(str(path), dtype=np.float32)
    except Exception as e:
        if self.logger: self.logger.warning(f"[IO] fail to read {path}: {e}")
        return None
    if raw.size == 0 or raw.size % nchan != 0:
        if self.logger: self.logger.warning(f"[SHAPE] {path} size={raw.size}, expect multiple of {nchan}")
        return None
    pts = raw.reshape(-1, nchan)
    if not np.isfinite(pts).all():
        if self.logger: self.logger.warning(f"[NaN/Inf] {path} contains non-finite values, nan_to_num applied")
        pts = np.nan_to_num(pts, nan=0.0, posinf=1e4, neginf=-1e4)
    return pts

def _ensure_feat_dim(self, pts: np.ndarray, expect: int):
    """把特征维度对齐到 expect：多了截断、少了补零；保持 float32。"""
    if pts is None: return None
    if pts.shape[1] == expect: return pts.astype(np.float32, copy=False)
    if pts.shape[1] > expect:
        return pts[:, :expect].astype(np.float32, copy=False)
    pad = np.zeros((pts.shape[0], expect - pts.shape[1]), dtype=np.float32)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_dual_radar_infos':
        ...
        create_dual_radar_infos(
            ...
            data_path= Path('/root/autodl-tmp/data'),
            save_path= Path('/root/autodl-tmp/data')
        )
