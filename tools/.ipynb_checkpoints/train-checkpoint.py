...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default='cfgs/VoD_models/L4DR.yaml', help='specify the config for training')
    ...
    parser.add_argument('--extra_tag', type=str, default='l4dr_demo', help='extra tag for this experiment')
...
def main():
    ...
    # ===== Debug hooks: check NaN/Inf =====
    DEBUG_CHECK_NAN = True
    _hook_handles = []

    if DEBUG_CHECK_NAN:
        torch.autograd.set_detect_anomaly(True)

        def _check_module(mod, inp, out):
            def _bad(x):
                return torch.is_tensor(x) and not torch.isfinite(x).all()

            bad = []
            for name, t in (("input", inp), ("output", out)):
                if isinstance(t, (tuple, list)):
                    if any(_bad(xx) for xx in t if torch.is_tensor(xx)):
                        bad.append(name)
                elif _bad(t):
                    bad.append(name)
            if bad:
                raise RuntimeError(f"[NaN/Inf] at {mod.__class__.__name__} -> {bad}")

        for m in model.modules():
            _hook_handles.append(m.register_forward_hook(_check_module))
