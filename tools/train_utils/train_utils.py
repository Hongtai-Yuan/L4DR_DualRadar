...
# ----------------------- NaN/Inf 处理工具函数 -----------------------
def _nan_to_num_in_batch(batch, keys=('points', 'voxels', 'voxels_l', 'voxels_r',
                                      'voxel_features', 'pillar_features')):
    """
    就地把 batch 里常见的大张量做 nan/inf 清洗（仅当这些键存在且为 Tensor）。
    """
    for k in keys:
        v = batch.get(k, None)
        if torch.is_tensor(v):
            batch[k] = torch.nan_to_num(v, nan=0.0, posinf=1e6, neginf=-1e6)


def _assert_finite_batch(batch, where=''):
    """
    严格检查 batch 内所有 Tensor 是否全为有限值；否则抛出带定位信息的异常。
    """
    for k, v in batch.items():
        if torch.is_tensor(v):
            if not torch.isfinite(v).all():
                bad_idx = (~torch.isfinite(v)).nonzero(as_tuple=False)
                preview = bad_idx[:5].tolist()  # 避免刷屏
                shape = tuple(v.shape)
                raise RuntimeError(f"[NaN/Inf] in {where}:{k} shape={shape} examples_idx={preview}")


# 通过环境变量控制策略：check（默认）或 clamp
_NAN_ACTION = os.getenv('L4DR_NAN_ACTION', 'check').lower()
...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        # 进模型前的 NaN/Inf 检查或清洗
        if _NAN_ACTION == 'clamp':
            _nan_to_num_in_batch(batch)
        else:
            _assert_finite_batch(batch, where='batch_input')
        ...
        if not torch.isfinite(loss):
            if _NAN_ACTION != 'clamp':
                _nan_to_num_in_batch(batch)  # 辅助判断是否来源于输入
            raise RuntimeError(f"[NaN/Inf] in loss at iter={accumulated_iter}, epoch={cur_epoch}")
        ...
        if rank == 0:
            ...
            if tb_log is not None:
                tb_log.add_scalar('train/loss', float(loss.detach().item()) if torch.isfinite(loss) else float('nan'),
                                  accumulated_iter)
                tb_log.add_scalar('meta_data/learning_rate', cur_lr, accumulated_iter)
